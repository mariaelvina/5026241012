...
@section('konten')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('kursi.store') }}" method="POST" onsubmit="return validasiForm()">
                @csrf

                <div class="row mb-3">
                    <label for="merkkursi" class="col-sm-2 col-form-label">Merk Kursi</label>
                    <div class="col-sm-10">
                        <input type="text" name="merkkursi" id="merkkursi" class="form-control" maxlength="30" value="{{ old('merkkursi') }}" placeholder="Masukkan merk kursi...">
                    </div>
                </div>
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="number" name="stockkursi" id="stockkursi" class="form-control" min="0" value="{{ old('stockkursi') }}" placeholder="0">
                    </div>
                </div>
                    ...
                    <div class="col-sm-10 d-flex align-items-center">
                        <div class="form-check form-switch fs-5">
                            <input class="form-check-input" type="checkbox" role="switch" id="toggleTersedia"
                                   {{ old('tersedia', 'Y') == 'Y' ? 'checked' : '' }}>
                            <label class="form-check-label text-muted ms-2" id="labelTersedia">Ya</label>
                        </div>
                        <input type="hidden" name="tersedia" id="tersedia" value="{{ old('tersedia', 'Y') }}">
                    </div>
                </div>
